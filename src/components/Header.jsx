...
const Header = () => {
    ...
    return (
        <header className="header px-4 lg:px-6 py-5">
            <div className="container">
                <div className="flex justify-between items-center mx-auto max-w-screen-xl">
                    {/* Logo */}
                    <Link to="/" className="text-xl font-semibold text-[#000]">
                        ...
                        {navMenu.map((item, index) => (
                            <div key={index} className="relative">
                                {item.path !== "#" ? (
                                    <Link
                                        to={item.path}
                                        className="py-2 px-4 text-gray-700 hover:text-primary-700 text-[#000]"
                                    >
                                        {item.name}
                                    </Link>
                                ) : (
                                    <button
                                        className="flex items-center  text-gray-700 hover:text-primary-700 text-[#000]"
            ...
            {isMobileMenuOpen && (
                <div className="lg:hidden absolute top-16 left-0 w-full bg-white shadow-md dark:bg-gray-800 transition-all duration-300">
                    <nav className="flex flex-col items-start px-4 py-4 space-y-2">
                        {navMenu.map((item, index) => (
                            <div key={index} className="w-full">
                                ...
                                {item.subMenu && dropdown === index && (
                                    <ul className="bg-gray-100 dark:bg-gray-700 rounded-md">
                                        {item.subMenu.map((subItem, subIndex) => (
                                            <li key={subIndex}>
                                                <Link
                                                    to={subItem.path}
